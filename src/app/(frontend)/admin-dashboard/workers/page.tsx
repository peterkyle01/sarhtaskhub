...
import { Search, Plus, Edit, Trash2, Eye, Users, UserCheck, FileText, Home, BarChart3, ChevronLeft, ChevronRight, Star } from 'lucide-react'
...
const specialtyOptions = [
  ...
  "Data Analysis", "Algebra", "Geometry", "Calculus", "Linear Algebra",
...
export default function WorkersPage() {
  ...
  const filteredWorkers = mockWorkers.filter(worker => {
    ...
    const matchesPerformance = performanceFilter === "all" || 
                              ...
                              (performanceFilter === "good" && worker.performanceScore >= 80 && worker.performanceScore < 90) ||
                              (performanceFilter === "average" && worker.performanceScore >= 70 && worker.performanceScore < 80) ||
  ...
  const handleViewTaskHistory = (worker: any) => {
  ...
  const mockTaskHistory = [
    { id: "T001", clientName: "John Smith", courseName: "Calculus I", status: "Completed", completedDate: "2024-01-15", rating: 5 },
    { id: "T002", clientName: "Emily Johnson", courseName: "Physics II", status: "Completed", completedDate: "2024-01-20", rating: 4 },
    { id: "T003", clientName: "Michael Brown", courseName: "Statistics", status: "In Progress", completedDate: null, rating: null },
  ]
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    ...
                    <div className="grid gap-4 py-4">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Select value={newWorker.status} onValueChange={(value) => setNewWorker({...newWorker, status: value})}>
                          <SelectTrigger>
                          ...
                          {specialtyOptions.map((specialty) => (
                            <label key={specialty} className="flex items-center space-x-2 text-sm">
                              <input
                                ...
                                onChange={(e) => {
                                  if (e.target.checked) {
                                    setNewWorker({...newWorker, specialties: [...newWorker.specialties, specialty]})
                                  } else {
                                    setNewWorker({...newWorker, specialties: newWorker.specialties.filter(s => s !== specialty)})
                                  }
                                }}
                                className="rounded"
                              />
                    ...
                    {paginatedWorkers.map((worker) => (
                      <TableRow key={worker.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Progress 
                              value={worker.performanceScore} 
                              className="w-16 h-2"
                            />
                            <span className="text-sm font-medium w-8">{worker.performanceScore}%</span>
                            <span className={`text-xs px-2 py-1 rounded-full ${
                              worker.performanceScore >= 90 ? 'bg-green-100 text-green-800' :
                              worker.performanceScore >= 80 ? 'bg-blue-100 text-blue-800' :
                              worker.performanceScore >= 70 ? 'bg-yellow-100 text-yellow-800' :
