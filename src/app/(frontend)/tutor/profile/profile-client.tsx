...
interface Props {
  initialProfile: TutorDoc
  availableSubjects: Subject[]
}

export function TutorProfileClient({ initialProfile, availableSubjects }: Props) {
  ...
  function handleProfileUpdate() {
    ...
    startTransition(async () => {
      try {
        const updated = await updateCurrentTutorProfile(profileData)
        if (updated) {
          setProfile(updated)
  ...
  function handlePasswordUpdate() {
    ...
    startTransition(async () => {
      try {
        const result = await updateCurrentTutorPassword(
          passwordData.currentPassword,
          passwordData.newPassword,
        )
    ...
    uploadProfilePicture(formData)
      .then(async (result) => {
        if (result.success && result.mediaId) {
          ...
          const updatedProfile = await updateCurrentTutorProfile({
            ...profileData,
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-muted-foreground">Active Tutor</span>
                </div>
                <div className="mt-2">
                  <Button
                    variant="outline"
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Max 1MB • JPEG, PNG, WebP</p>
                </div>
              </div>
            </div>
              ...
              <div className="grid gap-2">
                ...
                <div className="space-y-2">
                  ...
                  {selectedSubjects.length === 0 && (
                    <p className="text-sm text-muted-foreground">Select subjects you teach</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {selectedSubjects.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Subjects you&apos;re currently assigned to teach</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2">
